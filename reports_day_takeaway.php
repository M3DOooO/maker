...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_day.php?day=<?php echo $rday;?>&month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير يوم <?php echo $ryear?>-<?php echo $rmonth?>-<?php echo $rday?></span></a> / <span>فواتير الطلبات الخارجية</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(price) FROM reports2 WHERE day = $rday AND month = $rmonth AND year = $ryear  AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done'"; 
...
$result = mysql_query("SELECT *,SUM(price),SUM(discount2) FROM `reports2` WHERE day = $rday AND month = $rmonth AND year = $ryear AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
