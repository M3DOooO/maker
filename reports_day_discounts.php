...
<div style="border-style:solid;border-width:1px;padding:15px;margin-right: 50px;">
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_day.php?day=<?php echo $rday;?>&month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير يوم <?php echo $ryear?>-<?php echo $rmonth?>-<?php echo $rday?></span></a> / <span>التخفيضات</span>
			</div>
<div class="row-fluid">		
				<div class="box span11">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> <?php echo $lang_105;?> في فواتير اللعب</h2>
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE day = $rday AND month = $rmonth AND year = $ryear AND dis_reason !=''"; 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE day = $rday AND month = $rmonth AND year = $ryear AND End_hour != '-'  AND (discount2 >0 OR discount_amount >0) GROUP BY session_id"); 
?><thead>
<tr>
...
// To connect to the database
$result = mysql_query("SELECT *,SUM(price),SUM(discount2) FROM `reports2` WHERE day = $rday AND month = $rmonth AND year = $ryear   AND dis_reason !='' GROUP BY session_id"); 
?><thead>
<tr>
								  <th><?php echo $lang_149;?></th>
                                  <th><?php echo $lang_150;?></th>
