...
<!-- left menu ends -->
			...
			<a href="control.php"><span>لوحة التحكم</span></a> / <span>إضافة كميات لأصناف قديمة</span>
			</div>
			<br/>
			<?php 
...
<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> <?php echo $lang_385;?></h2>
					...
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable">
						  ...
						  <tbody>
							<tr>
							<form action = "actions/products/update_bulk.php" method="post">
							
								<td>    <?php  mysql_connect("$host", "$user", "$pass")or die("cannot connect");
      mysql_select_db("$db")or die("cannot select DB");
      $result = mysql_query("SELECT * FROM `stock` where name !=' ' GROUP BY name ORDER BY name ASC"); 
	  ?>
	  <div class="controls">
 <select name="choose_name"  >

 <option value="no"><?php echo $lang_396;?></option>
 <?php 
      while($row = mysql_fetch_array($result))
      {
   ?>
...
?>
    ...
    <td>  <input   id="dare_price" name="choose_cost"  type="text" /></td>
...
?>
    ...
    <td>  <input   id="dare_price2" name="choose_cost2"  type="text" /></td>
...
?>
    ...
    <td>  <input   id="dare_price3" name="choose_cost3"  type="text" /></td>
...
?>
    ...
    <td>  <input   id="dare_price4" name="choose_cost4"  type="text" /></td>
...
?>
    ...
    <td>  <input   id="dare_price5" name="choose_cost5"   type="text" /></td>
<td><input  name="total_cost5" id="total_price_amount5" readonly="readonly" value=""/></td>
				...
				<br/>
				<center>
				<button class="btn btn-success"><?php echo $lang_395;?></button>
				</center>
				</form>
				</div>
