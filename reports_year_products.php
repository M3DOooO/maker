...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $Y;?>"><span>تقارير سنة <?php echo $Y?></span></a> / <span>تقارير الأصناف</span>
			</div>
			<!-- content starts -->
...
<div class="row-fluid">		
				<div class="box span11">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> تقرير الأصناف بالسنة </h2>
																...
																while($row = mysql_fetch_array($result))
{
	  ...
	  $result4 = mysql_query("SELECT *,SUM(num),SUM(price) FROM `ps_orders` WHERE  `year` = $Y AND `name` = '$thename'");
	  	while($roww4 = mysql_fetch_array($result4))
{
	...
	$result2 = mysql_query("SELECT *,SUM(num),SUM(price) FROM `reports2` WHERE year = $Y AND `catagory` != 'in' AND `catagory` != 'exp' AND `name` = '$thename'");
	  	while($roww = mysql_fetch_array($result2))
{
	...
	$price2 = $roww['SUM(price)'];
	  ...
	  $result3 = mysql_query("SELECT SUM(notes) FROM `reports2` WHERE `name` = '$thename' AND `year` = '$Y' AND `catagory` = 'exp' ");
...
<td><a target="_blank" href='reports_year_items_out.php?name=<?php echo $row['name'];?>&&year=<?php echo $Y;?>'><?php echo $xxx;?></a></td>
<td><a target="_blank" href="reports_year_items_in.php?name=<?php echo $row['name'];?>&&year=<?php echo $Y;?>"><?php echo $sum3;?></a></td>
