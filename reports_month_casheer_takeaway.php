...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير شهر <?php echo $ryear?>-<?php echo $rmonth?></span></a> / <a href="reports_month_casheer.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير الكاشير</span></a> / <a href="reports_month_casheer_all.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>&se_cash=<?php echo $Rcash;?>"><span><?php echo $Rcash;?> </span></a> / <span>فواتير الطلبات الخارجية</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(price) FROM reports2 WHERE month = $rmonth AND year = $ryear  AND `casheer` = '$Rcash' AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done'"; 
...
$result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
