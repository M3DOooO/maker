...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");
$result = mysql_query("SELECT * FROM `ps_orders` WHERE  year = $Y AND name = '$getname'");
?><thead>
<tr>
								  <th>نوع الفاتورة</th>
...
while($row = mysql_fetch_array($result))
{
  ...
  echo "</tr>";
  	...
  	$result2 = mysql_query("SELECT * FROM `reports2` WHERE  year = $Y AND `catagory` != 'in' AND `catagory` != 'exp' AND `name` = '$getname'");
