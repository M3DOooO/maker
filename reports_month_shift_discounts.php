...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير شهر <?php echo $ryear?>-<?php echo $rmonth?></span></a> / <a href="reports_month_shift.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير الشفت</span></a> / <a href="reports_month_shift_all.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>&se_shift=<?php echo $rshift;?>"><span>الشفت <?php echo $eeshift;?> </span></a> / <span>التخفيضات</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE  month = $rmonth AND year = $ryear AND `shift` = '$rshift' AND dis_reason !='' AND status = 'done'"; 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE  month = $rmonth AND year = $ryear AND End_hour != '-'  AND (discount2 >0 OR discount_amount >0) AND `shift` = '$rshift' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
...
// To connect to the database
$result = mysql_query("SELECT *,SUM(price),SUM(discount2) FROM `reports2`
 WHERE  month = $rmonth 
 AND year = $ryear 
 AND `shift` = '$rshift'  
 AND dis_reason !='' GROUP BY session_id"); 
?><thead>
<tr>
								  <th><?php echo $lang_149;?></th>
                                  <th><?php echo $lang_150;?></th>
