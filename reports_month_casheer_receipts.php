...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير شهر <?php echo $ryear?>-<?php echo $rmonth?></span></a> / <a href="reports_month_casheer.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير الكاشير</span></a> / <a href="reports_month_casheer_all.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>&se_cash=<?php echo $Rcash;?>"><span><?php echo $Rcash;?> </span></a> / <span>فواتير الأجهزة</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE  month = $rmonth AND year = $ryear  AND `casheer` = '$Rcash'  AND status= 'done'"; 
...
$result_count = mysql_query("SELECT COUNT(*) as total FROM `reports` WHERE month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND End_hour != '-' AND status = 'done' GROUP BY session_id"); 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND End_hour != '-' AND status = 'done' GROUP BY session_id LIMIT $offset, $items_per_page"); 
 ?><thead>
<tr>
			...
			<div class="row-fluid">
				<div class="span11" style="text-align:center; margin-top:20px;">
					<ul class="pagination">
						...
						// الزر السابق
						if($current_page > 1) {
							echo '<li><a href="reports_month_casheer_all.php?month='.$rmonth.'&year='.$ryear.'&se_cash='.$Rcash.'&page='.($current_page-1).'">« السابق</a></li>';
						} else {
						...
						for($i = 1; $i <= $total_pages; $i++) {
							if($i == $current_page) {
								...
								echo '<li><a href="reports_month_casheer_all.php?month='.$rmonth.'&year='.$ryear.'&se_cash='.$Rcash.'&page='.$i.'">'.$i.'</a></li>';
							}
						}

						// الزر التالي
						if($current_page < $total_pages) {
							echo '<li><a href="reports_month_casheer_all.php?month='.$rmonth.'&year='.$ryear.'&se_cash='.$Rcash.'&page='.($current_page+1).'">التالي »</a></li>';
						} else {
