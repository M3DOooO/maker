...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير شهر <?php echo $ryear?>-<?php echo $rmonth?></span></a> / <a href="reports_month_shift.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير الشفت</span></a> / <a href="reports_month_shift_all.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>&se_shift=<?php echo $rshift;?>"><span>الشفت <?php echo $eeshift;?> </span></a> / <span>فواتير الأجهزة</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE  month = $rmonth AND year = $ryear  AND `shift` = '$rshift' AND status= 'done'"; 
...
$result = mysql_query("SELECT *, SUM(money) as sum_money_rows, SUM(total) as sum_total_rows, MIN(Start_hour*60 + Start_minute) as min_start_minute, MAX(End_hour*60 + End_minute) as max_end_minute FROM `reports` WHERE  month = $rmonth AND year = $ryear AND `shift` = '$rshift' AND End_hour != '-'  AND status= 'done' GROUP BY session_id"); 
 ?><thead>
<tr>
