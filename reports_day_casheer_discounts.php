...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE day = $rday AND month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND dis_reason !=''"; 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE day = $rday AND month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND End_hour != '-'  AND (discount2 >0 OR discount_amount >0) GROUP BY session_id"); 
?><thead>
<tr>
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE day = $rday AND month = $rmonth AND year = $ryear AND dis_reason !='' AND `casheer` = '$Rcash'"; 
$resulty = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($resulty)){
$items = $row['SUM(money)'];  
...
// To connect to the database
$result = mysql_query("SELECT *,SUM(price),SUM(discount2) FROM `reports2` WHERE day = $rday AND month = $rmonth AND year = $ryear AND `casheer` = '$Rcash'  AND dis_reason !='' GROUP BY session_id"); 
?><thead>
<tr>
								  <th><?php echo $lang_149;?></th>
                                  <th><?php echo $lang_150;?></th>
