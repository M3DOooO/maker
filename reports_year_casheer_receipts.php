...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $ryear;?>"><span>تقارير سنة <?php echo $ryear?></span></a> / <a href="reports_year_casheer.php?year=<?php echo $ryear;?>"><span>تقارير الكاشير</span></a> / <a href="reports_year_casheer_all.php?year=<?php echo $ryear;?>&se_cash=<?php echo $Rcash?>"><span><?php echo $Rcash?></span></a> / <span>فواتير الأجهزة</span>
			</div>
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");
$query = "SELECT  SUM(money) FROM reports WHERE  year = $ryear  AND `casheer` = '$Rcash'  AND status= 'done'";
...
$result = mysql_query("SELECT *, SUM(money) as sum_money_rows, SUM(total) as sum_total_rows, MIN(Start_hour*60 + Start_minute) as min_start_minute, MAX(End_hour*60 + End_minute) as max_end_minute FROM `reports` WHERE year = $ryear AND `casheer` = '$Rcash'  AND End_hour != '-'  AND status= 'done' GROUP BY session_id ORDER BY max_end_minute");
 ?><thead>
<tr>
