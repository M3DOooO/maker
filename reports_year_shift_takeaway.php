...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(price) FROM reports2 WHERE year = $ryear AND `shift` = '$rshift' AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done'"; 
...
$result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE year = $ryear AND `shift` = '$rshift' AND `catagory` != 'in' AND `catagory` != 'exp' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
