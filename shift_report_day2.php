...
<tr>
						...
						<?php
// الوقت
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE day = '$D' AND month = '$M'  AND year = '$Y' AND End_hour != '-' AND status = 'done'  AND shift = '$selectshift' GROUP BY shift ORDER BY MAX(End_hour*60 + End_minute)");
while($row = mysql_fetch_array($result))
{
