...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير شهر <?php echo $ryear?>-<?php echo $rmonth?></span></a> / <a href="reports_month_shift.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>"><span>تقارير الشفت</span></a> / <a href="reports_month_shift_all.php?month=<?php echo $rmonth;?>&year=<?php echo $ryear;?>&se_shift=<?php echo $rshift;?>"><span>الشفت <?php echo $eeshift;?> </span></a> / <span>فواتير الطلبات الخارجية</span>
			</div>
			<!-- content starts -->
...
<div class="row-fluid">		
				<div class="box span11">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> <?php echo $lang_266;?> </h2>
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(price) FROM reports2 WHERE month = $rmonth AND year = $ryear  AND `catagory` != 'in' AND `catagory` != 'exp' AND `shift` = '$rshift' AND status = 'done' "; 
...
$result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE month = $rmonth AND year = $ryear AND `catagory` != 'in' AND `catagory` != 'exp' AND `shift` = '$rshift' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
