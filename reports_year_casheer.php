...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $Y;?>"><span>تقارير سنة <?php echo $Y?></span></a> / <span>تقارير الكاشير</span>
			</div>
			<!-- content starts -->
...
<h2><?php echo $lang_330;?>: <font color='green'><?php echo $money_all;?></font> <?php echo $lang_100;?> </h2> 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE year = '$Y' AND End_hour != '-' AND casheer !='' AND status = 'done' AND casheer = '$cahseer' ");						
while($row = mysql_fetch_array($result))
{
	$money1 = $row['SUM(money)'];
 	 // المصاريف
$result2 = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE year = '$Y'  AND catagory ='exp' AND casheer = '$cahseer' AND status ='done'");
while($row2 = mysql_fetch_array($result2))
{
$money4 = $row2['SUM(price)'];
}	
     //المشتروات غير المصاريف
$result3 = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE year = '$Y'  AND catagory !='exp' AND catagory !='in' AND casheer = '$cahseer' AND status = 'done'");
while($row3 = mysql_fetch_array($result3))
{
$money2 = $row3['SUM(price)'];
}
     //الأوردرات
$result4 = mysql_query("SELECT *,SUM(price) FROM `ps_orders` WHERE year = '$Y'  AND casheer = '$cahseer' AND status ='yes'");
while($row4 = mysql_fetch_array($result4))
{
$money3 = $row4['SUM(price)'];
}
     //الخصم
$result5 = mysql_query("SELECT SUM(discount2) FROM `reports` WHERE year = '$Y' AND casheer = '$cahseer' AND status = 'done'");	
while($row5 = mysql_fetch_array($result5))
{
$money5 = $row5['SUM(discount2)'];
} 
     //الخصم 2
$result6 = mysql_query("SELECT SUM(discount_amount) FROM `reports` WHERE year = '$Y' AND casheer = '$cahseer' AND status = 'done'");
while($row6 = mysql_fetch_array($result6))
{
$money6 = $row6['SUM(discount_amount)'];
}
     //المشتروات غير المصاريف
$result7 = mysql_query("SELECT SUM(price) FROM `reports2` WHERE year = '$Y'   AND catagory ='in' AND casheer = '$cahseer' AND status ='done'");
while($row7 = mysql_fetch_array($result7))
{
$money7 = $row7['SUM(price)'];
}
   $query = "SELECT  SUM(discount2) FROM reports2 where   year = $Y AND status = 'done'  AND casheer = '$cahseer'"; 
$resulty = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($resulty)){
$ds5 = $row['SUM(discount2)'];
...
<td><a href="reports_year_casheer_all.php?se_year=<?php echo $Y;?>&se_cash=<?php echo $cahseer;?>"><img src="img/app/buttons/info.png"></a></td><?php 
      echo "</tr>";
}}?>
						  </tbody>
<?php 
...
<tr>
						...
						<?php 	
// الوقت
 $result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE year = '$Y'  AND catagory !='exp' AND catagory !='in' AND casheer !='' AND status = 'done' GROUP BY casheer");					
while($row = mysql_fetch_array($result))
{
	$money1 = 0;
	$cahseer = $row['casheer'];
...
<td><h3><font color="red"><?php echo $money5+$money6+$ds5;?></font></h3></td>
<td><h2><?php echo $moneyall;?></h2></td>
<td><a href="reports_year_casheer_all.php?se_year=<?php echo $Y;?>&se_cash=<?php echo $row['casheer'];?>"><img src="img/app/buttons/info.png"></a></td><?php 
      echo "</tr>";
  }?>
						  </tbody>
<?php 
