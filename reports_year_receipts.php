...
// ✅ Pagination
$items_per_page = 100; // عدد الأوردرات لكل صفحة
...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $ryear;?>"><span>تقارير سنة <?php echo $ryear?></span></a> / <span>فواتير الأجهزة</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE  year = $ryear  AND status = 'done'"; 
...
$result_count = mysql_query("SELECT COUNT(*) as total FROM `reports` WHERE year = $ryear AND End_hour != '-' AND status = 'done' GROUP BY session_id"); 
...
$result = mysql_query("SELECT *, SUM(money) as sum_money_rows, SUM(total) as sum_total_rows, MIN(Start_hour*60 + Start_minute) as min_start_minute, MAX(End_hour*60 + End_minute) as max_end_minute FROM `reports` WHERE year = $ryear AND End_hour != '-' AND status = 'done' GROUP BY session_id LIMIT $offset, $items_per_page"); 
 ?><thead>
<tr>
			...
			<div class="row-fluid">
				<div class="span11" style="text-align:center; margin-top:20px;">
					<ul class="pagination">
						...
						// الزر السابق
						if($current_page > 1) {
							echo '<li><a href="reports_year.php?year='.$ryear.'&page='.($current_page-1).'">« السابق</a></li>';
						} else {
						...
						for($i = 1; $i <= $total_pages; $i++) {
							if($i == $current_page) {
								...
								echo '<li><a href="reports_year.php?year='.$ryear.'&page='.$i.'">'.$i.'</a></li>';
							}
						}

						// الزر التالي
						if($current_page < $total_pages) {
							echo '<li><a href="reports_year.php?year='.$ryear.'&page='.($current_page+1).'">التالي »</a></li>';
						} else {
