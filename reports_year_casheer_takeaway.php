...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $ryear;?>"><span>تقارير سنة <?php echo $ryear?></span></a> / <a href="reports_year_casheer.php?year=<?php echo $ryear;?>"><span>تقارير الكاشير</span></a> / <a href="reports_year_casheer_all.php?year=<?php echo $ryear;?>&se_cash=<?php echo $Rcash?>"><span><?php echo $Rcash?></span></a> / <span>فواتير الطلبات الخارجية</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(price) FROM reports2 WHERE year = $ryear  AND `catagory` != 'in' AND `catagory` != 'exp' AND `casheer` = '$Rcash' AND status = 'done'"; 
...
$result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE year = $ryear AND `catagory` != 'in' AND `catagory` != 'exp'  AND `casheer` = '$Rcash' AND status = 'done' GROUP BY session_id"); 
?><thead>
<tr>
