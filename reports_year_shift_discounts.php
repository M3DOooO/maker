...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_year.php?year=<?php echo $ryear;?>"><span>تقارير سنة <?php echo $ryear?></span></a> / <a href="reports_year_shift.php?year=<?php echo $ryear;?>"><span>تقارير الشفت</span></a> / <a href="reports_year_shift_all.php?year=<?php echo $ryear;?>&se_shift=<?php echo $rshift?>"><span>الشفت <?php echo $eeshift?></span></a> / <span>التخفيضات</span>
			</div>
			<!-- content starts -->
...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE year = $ryear  AND `shift` = '$rshift' AND (discount2 >0 OR discount_amount >0)"; 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE year = $ryear AND `shift` = '$rshift' AND End_hour != '-'  AND (discount2 >0 OR discount_amount >0) GROUP BY session_id"); 
?><thead>
<tr>
...
// To connect to the database
$result = mysql_query("SELECT *,SUM(price),SUM(discount2) FROM `reports2` WHERE year = $ryear  AND `shift` = '$rshift'  AND dis_reason !='' GROUP BY session_id"); 
?><thead>
<tr>
								  <th><?php echo $lang_149;?></th>
                                  <th><?php echo $lang_150;?></th>
