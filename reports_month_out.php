...
<div style="border-style:solid;border-width:1px;padding:15px;margin-right: 50px;">
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_month.php?month=<?php echo $M;?>&year=<?php echo $Y;?>"><span>تقارير شهر <?php echo $Y?>-<?php echo $M?></span></a> / <span>المصاريف</span>
			</div>
<!-- content starts -->
...
$query = "SELECT  SUM(price) FROM reports2 where catagory = 'exp' AND month = '$M' AND year = '$Y' "; 
...
$result = mysql_query("SELECT * FROM `reports2` WHERE catagory  = 'exp' AND month = '$M' AND year = '$Y' ");
?><thead>
<tr>
...
$shift_check = $row['shift'];
 if($shift_check == 'One'){$shift_check2= $lang_155;}else{$shift_check2 = $lang_156;}
	...
	?><td class="center">
	<?php if($row['price'] == 0) { ?>
		<span class="label label-important"><a href="reports_month_out.php?month=<?php echo $M;?>&year=<?php echo $Y;?>&etype=exp&&id=<?php echo $eid;?>" ><?php echo $lang_167;?></a></span>
	<?php } ?>
	</td>  <?php 
	echo "</tr>";
}?>
